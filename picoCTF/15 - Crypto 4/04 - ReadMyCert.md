...
Take a look at this CSR file [here](https://artifacts.picoctf.net/c/421/readmycert.csr).
## Solución
El archivo entregado es un CSR (certificate signing requests), y si le aplicamos un cat este es su contenido:
```
...
Ahora, obtengamos todos los detalles del CSR utilizando el comando ``openssl req -in readmycert.csr -noout -text``, el cual mostrará la información detallada del CSR, incluyendo el subject, la organización, el common name y otros datos relevantes.

El comando se compone de esto:
- openssl: Es la herramienta que usaremos para realizar la operación criptográfica
- req: Subcomando específico para gestionar CSR
- -in (archivo): Claramente, le especificamos el archivo a procesar
- -noout: Evita que se genere la salida del certificado codificado, mostrando solo la información solicitada
- -text: Muestra el contenido del CSR en un formato legible
```
┌──(kali㉿kali)-[/media/sf_almacenamientoCompartido/Crypto4/ReadMyCert]
...
Y como vemos, nos muestra nuestra flag
